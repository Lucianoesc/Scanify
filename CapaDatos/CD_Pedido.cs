...
namespace CapaDatos
{
    public class CD_Pedido
    {
        ...
        public List<Pedido> ObtenerReportePedido(string fechaInicio, string fechaFin)
        {
            ...
            using (SqlConnection conexion = new SqlConnection(Conexion.cadena))
            {
                try
                {
                    SqlCommand cmd = new SqlCommand("sp_ReportePedido", conexion);
                    ...
                    using (SqlDataReader dr = cmd.ExecuteReader())
                    {
                        while (dr.Read())
                        {
                            ...
                            {
                                IdPedido = Convert.ToInt32(dr["IdPedido"]),
                                HoraPedido = dr["HoraPedido"].ToString(),
                                FechaRegistro = dr["FechaRegistro"].ToString(),
                                NumeroDocumento = dr["NumeroDocumento"].ToString(),
        ...
        public bool ModificarEstadoPedido(int idPedido, string nuevoEstado)
        {
            bool respuesta = false;
            ...
            using (SqlConnection conexion = new SqlConnection(Conexion.cadena))
            {
                try
                {
                    SqlCommand cmd = new SqlCommand("sp_ModificarEstadoPedido", conexion);
                    cmd.CommandType = CommandType.StoredProcedure;
