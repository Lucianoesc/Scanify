...
namespace CapaPresentacion
{
    public partial class FrmLogin : Form
    {
        ...
        private void btnactivo_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("La contraseña debe tener al menos 8 caracteres, incluir al menos un número y al menos una mayúscula.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            ...
            {
                string concatenado = usuario.Documento + usuario.Nombre + usuario.Apellido + usuario.Correo + usuario.Clave + usuario.Telefono + usuario.oRol.IdRol.ToString() + (usuario.Estado ? "1" : "0") + usuario.Username;
                byte[] bytes = Encoding.UTF8.GetBytes(concatenado); // Convertir la cadena en una secuencia de bytes
                int suma = bytes.Sum(b => (int)b); // Sumar los valores de los bytes
                char digitoVerificadorCalculado = (char)((suma % 10) + '0');

                // Comparar el dígito verificador calculado con el almacenado en la base de datos
                if (digitoVerificadorCalculado == usuario.DigitoVerificador)
                {
                    // Dígito verificador correcto, continuar con el login
                    ...
                    dash.FormClosed += FrmLogin_FormClosed;
                    dash.Show();
                ...
                else
                {
                    ...
                    this.Alerta("Dígito verificador incorrecto", Form_Alertas.enmType.Error);
                    cnBitacora.InsertarBitacora(username, usuario.IdUsuario.ToString(), "Login", "Dígito verificador incorrecto", "FrmLogin");
                }
            }
            else
            {
                ...
                // Insertar en la bitácora el intento de inicio de sesión fallido
                int idUsuario = usuario != null ? usuario.IdUsuario : -1;
                cnBitacora.InsertarBitacora(username, idUsuario.ToString(), "Login", "Login error", "FrmLogin");
            }
        }
        ...
        private bool EsContraseñaValida(string contraseña)
        {
            ...
            return Regex.IsMatch(contraseña, @"^(?=.*\d)(?=.*[A-Z]).{8,}$");
        }
        private void label3_Click(object sender, EventArgs e)
        {
            FrmRecuperarContraseña frm = new FrmRecuperarContraseña(usuarioActual);
            frm.ShowDialog();
